...
The overlay is self-contained from chart OHLCV and TradingView time/session context. It creates line, label, box, table, plot, and plotshape output for prior day/week/month levels, 20D/50D high-low levels, confirmed swing highs/lows, gap zones, ATR compression range boxes, intraday premarket/opening-range levels, and anchored VWAP from a major gap or confirmed pivot. It exposes `focus`, `clean`, `levels`, and `full-debug` style presets and uses timeframe checks to emphasize weekly, daily, and 65-minute review contexts. `focus` is the default quieter preset: horizontal reference and swing levels render as restrained 1 px solid lines with short text-only labels, weekly charts show major context, daily charts show uptrend pullback/reclaim context, and 65-minute charts show timing context while preserving price-scale plot output for extraction.
